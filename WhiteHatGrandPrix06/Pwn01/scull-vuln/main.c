...
int scull_shift(struct scull_dev *dev, int n_shift) {
  ...
  if (n_shift == 0 || qset < 2 || n_shift >= qset)
    return -1;
  PDEBUG("qset = %d\n", qset);
  ...
  for (dptr = dev->data; dptr; dptr = next) { // all the list items
    if (dptr->data) {
      for (i = 0, j = n_shift; j < qset; ++i, ++j)
        dptr->data[i] = dptr->data[j];
      cond_resched();
    }
    next = dptr->next;
  }
...
long scull_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
  ...
  case SCULL_IOCSHIFT:
    ...
    //     PDEBUG("buff = %px\n", buff);
    //     KFREE(buff);
    //     struct sk_buff* skb = alloc_skb(dev->quantum - 320, GFP_KERNEL); //
    //     gfp_mask in unix_stream_sendmsg 0x7000c0 PDEBUG( "skb->data = %px,
    //     GFP_KERNEL = 0x%x\n", skb->data, GFP_KERNEL); kfree_skb(skb); buff =
    //     kvmalloc(dev->quantum, GFP_KERNEL); PDEBUG( "buff2 = %px\n", buff);
    //     KFREE(buff);
    //     break;
    // case SCULL_TEST_SMT:
    //     retval = *(int*)arg;
...
int scull_init_module(void) {
   ...
   */
  scull_devices =
      KMALLOC(scull_nr_devs * sizeof(struct scull_dev), KMALLOC_GFP_MASK);
  if (!scull_devices) {
    result = -ENOMEM;
